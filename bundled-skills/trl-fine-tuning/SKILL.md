...
## Practical Quick Start: DPO in 5 Lines
...
trainer = DPOTrainer(
    model=model,
    args=DPOConfig(output_dir="dpo_out", beta=0.1, per_device_train_batch_size=4),
    train_dataset=preference_dataset,  # chosen/rejected pairs
    processing_class=tokenizer
)
trainer.train()
...
|------|-------------------|-----|
| Ultra-fast LoRA (2-5x) | `mlops/training/unsloth` | Speed + memory + GRPO built-in |
| YAML-only declarative training | `mlops/training/axolotl` | No code needed |
| Structured generation for eval | `mlops/inference/outlines` | JSON schema from model outputs |
| Serve fine-tuned model | `mlops/inference/vllm` | High-throughput production serving |
| Local CPU inference | `mlops/inference/llama-cpp` | Convert to GGUF and run anywhere |

## Self-Test Scenarios

### Scenario 1: Full RLHF Pipeline (SFT → Reward → PPO)
- **Goal:** Train, reward-model, then reinforce a 7B model.
- **Steps:** SFTTrainer on instruction dataset → RewardTrainer on preference data → PPO with reward model.
- **Verify:** Reward model scores human-preferred completions higher. PPO policy generates aligned outputs.

### Scenario 2: Align with DPO (No Reward Model)
- **Goal:** Align model to preferences with just a dataset and 24GB VRAM.
- **Steps:** Load SFT model → DPOTrainer with `beta=0.1` → preference dataset with `chosen`/`rejected`.
- **Verify:** DPO loss decreases. Chosen completions score higher on implicit reward.

### Scenario 3: Online RL with GRPO
- **Goal:** Train reasoning on math problems with minimal VRAM.
- **Steps:** GRPOTrainer with `num_generations=4`. Custom reward: correctness + format. Low `per_device_train_batch_size`.
- **Verify:** Average reward per batch increases. Policy generates more correct answers.

### Scenario 4: Reward Hacking Detection
- **State:** Reward function only checks length → model outputs become verbose but irrelevant.
- **Action:** Update reward to include semantic metrics (e.g., BERTScore, factual overlap). Add KL penalty.
- **Verify:** Reward vs metric correlation plot shows gap shrinking.

### Scenario 5: Multi-GPU DPO
- **Goal:** Train DPO on 4xA100 with DeepSpeed.
- **Steps:** `accelerate config` → DeepSpeed Stage 2. DPOConfig with `bf16=True`. `accelerate launch`.
- **Verify:** All 4 GPUs show `~85%` utilization. Global batch size = per_device * 4. Synchronized checkpoints save.

## Version History

| Version | Date | Change |
|---------|------|--------|
| 1.0.0 | 2026-05-12 | Initial skill with TRL workflows, SFT/DPO/PPO/GRPO |
| 1.1.0 | 2026-05-13 | Added DPO 5-line quick start, cross-references to Unsloth/Axolotl/Outlines/vLLM/llama.cpp, scenarios, version history |
## Resources
