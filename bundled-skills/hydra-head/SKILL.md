...
This umbrella skill absorbed the following narrower siblings. See `scripts/` and `templates/` for their session-specific content:

| Absorbed Skill | Where its content lives | What it added |
|---|---|---|
| `hydra-head-operator` | `scripts/hydra-head-operator-*.sh` | Hydra node execution: init, commit, open, close, fanout |
| `hydra-head-troubleshooter` | `scripts/hydra-head-troubleshooter-*.sh`, `templates/hydra-head-troubleshooter-*.md` | Troubleshooting decision tree for "no head observed", peer sync, AckSn issues |
