...
- `cardano-cli-operator` (for deployment execution)

## Consolidated Skills

This umbrella skill absorbed the following narrower siblings. See `references/` and `templates/` for their session-specific content:

| Absorbed Skill | Where its content lives | What it added |
|---|---|---|
| `aiken-dex-security-audit` | `references/aiken-dex-security-audit-*.md`, `templates/aiken-dex-security-audit-*.md` | DEX security audit playbook with threat model, invariants, and exploit tx shapes |
| `aiken-dex-security-audit-operator` | (archived without support files) | Aiken build/test command runner for audit evidence |
