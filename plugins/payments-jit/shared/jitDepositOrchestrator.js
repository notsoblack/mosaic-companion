import { PaymentRequiredError } from '../../aim-nodes/shared/nodeManagerClient';
import { PolicyDeniedError, UserRejectedError } from './types';
...
export async function callPaidAimWithJit(args) {
    const { nodeUrl, slot, actionPath, payload = {}, currencyType = 'USDC', txDriver = 'ethereum', costOverrideBaseUnits, aimNodesClientFactory, paymentSigner, policy, logger = console, } = args;
    const client = aimNodesClientFactory();
    const doAimCall = () => client.callAim({
        slot,
        actionPath,
        payload,
        currencyType,
    ...
    try {
        ...
        try {
            const balanceData = await client.getBalance(sender);
        ...
        if (currentDailySpend + amount > policy.maxPerDay) {
            throw new PolicyDeniedError(`Top-up ${amount} would exceed maxPerDay limit of ${policy.maxPerDay} (current: ${currentDailySpend})`);
        }
        ...
        try {
            const verifyBalance = await client.getBalance(sender);
