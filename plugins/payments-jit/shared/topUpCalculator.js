 ...
 */
function extractCostFromCostOnly(costData, currencyType) {
    if (!costData)
        return 0n;
    ...
    if (directEntry && typeof directEntry === 'object' && !Array.isArray(directEntry)) {
        ...
        if (costValue > 0)
            return BigInt(costValue);
    }
    // Format B: costs array
    if (costData.costs && Array.isArray(costData.costs)) {
        const entry = costData.costs.find((c) => c.currency === currencyType)
...
export function calculateTopUpAmount(paymentDetails, policy, costOnlyData, currencyType = 'USDC') {
    ...
    if (requiredMissing === 0n && paymentDetails) {
        ...
        else if ((typeof paymentDetails.cost === 'string' || typeof paymentDetails.cost === 'number') &&
